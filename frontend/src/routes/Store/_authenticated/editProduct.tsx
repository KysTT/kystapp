...
export const Route = createFileRoute('/Store/_authenticated/editProduct')({
...
function GetProductForm(){
    ...
    const formGet = useForm({
        ...
        onSubmit: async ({value} ) => {
            ...
            const res = await api.store[`${product_id}`].$get()
    ...
    const formPut = useForm({
        ...
        onSubmit: async ({ value }) => {
            const res = await api.store[`${product_id}`].$put({ json: value })
            if (!res.ok) return toast('Bad request')
            formPut.reset()
            setProduct({
                name: "",
